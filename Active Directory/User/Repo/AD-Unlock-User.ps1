...
Function Get-UserStatus()
{
    ...
    $user = get-aduser -filter { samAccountName -eq $Username } -Credential $global:Creds -Properties * | Select-Object @{Name="BadPasswordTime";Expression={([datetime]::FromFileTime($_.badpasswordtime))}},badpwdCount,@{Name="LockoutTime";Expression={([datetime]::FromFileTime($_.lockoutTime))}},passwordLastSet
...
$DClist = Get-ADForest -Credential $Global:Creds | Select-object -ExpandProperty globalcatalogs | %{$_ | where-object {$_ -like "*$Domain*"} }
